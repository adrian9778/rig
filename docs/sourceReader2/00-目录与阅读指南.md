...
| 主题 | 路径 |
|------|------|
| Facade 门面 | `src/lib.rs`（特性门控重导出矩阵） |
| 核心契约 trait | `crates/rig-core/src/lib.rs`、`crates/rig-core/src/prelude.rs` |
| 客户端/Builder | `crates/rig-core/src/client/mod.rs` |
| 完成请求/响应 | `crates/rig-core/src/completion/{mod,request,message}.rs` |
| 流式 | `crates/rig-core/src/streaming/{mod,parts,identity}.rs` |
| HTTP 基础设施 | `crates/rig-core/src/http_client/{mod,sse,retry,multipart}.rs` |
| Provider trait 家族 | 见 `client/mod.rs` 与 `providers/mod.rs` |
| 内部适配层 | `crates/rig-core/src/providers/internal/`（17 个文件） |
| OpenAI Provider | `crates/rig-core/src/providers/openai/` |
| Gemini Provider | `crates/rig-core/src/providers/gemini/` |
| Agent 运行时 | `crates/rig-agent/src/agent/`（runner.rs / hook.rs / run/ / prompt_request/） |
| 上下文工具 | `crates/rig-agent/src/tool/` |
| 可移植工具 | `crates/rig-core/src/tool/` |
| `#[rig_tool]` | `crates/rig-derive/src/` |
| 向量库 trait | `crates/rig-core/src/vector_store/` |
| 记忆 trait | `crates/rig-core/src/memory.rs`、`crates/rig-memory/src/lib.rs` |
| 嵌入 | `crates/rig-core/src/embeddings/` |
| 错误类型 | `crates/rig-core/src/lib.rs` 与各 provider 模块 |
| WASM 兼容 | `crates/rig-core/src/wasm_compat.rs`、`crates/rig-core/src/markers.rs` |
| 遥测 | `crates/rig-core/src/telemetry/mod.rs` |
| 测试设施 | `crates/rig-core/src/test_utils/`、`tests/`（cassette 体系见 `tests/README.md`） |
